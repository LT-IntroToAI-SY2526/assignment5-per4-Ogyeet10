...
class Board:
    ...
    def update(self, row: int, col: int, assignment: int) -> None:
        ...
        """
        self.rows[row][col] = assignment
        self.num_nums_placed += 1
        for c in range(self.size):
            remove_if_exists(self.rows[row][c], assignment)

        for r in range(self.size):
            remove_if_exists(self.rows[r][col], assignment)

        subgrid_coords = self.subgrid_coordinates(row, col)
...
def DFS(state: Board) -> Board:
    ...
    while not stack.is_empty():
        ...
        row, col = board.find_most_constrained_cell()
        cell = board.rows[row][col]

        if isinstance(cell, list):
            for val in cell:
                next_board = copy.deepcopy(board)
                next_board.update(row, col, val)
                stack.push(next_board)
        # if it's already an int and not goal, continue loop
